...
\setlength{\cftbeforelottitleskip}{0pt plus 1pt} % sets the space before the list of tables to match sections
    ...
    \section{Future Work}
        \begin{enumerate}
            \item \textbf{Adding More Stations:} The app is currently limited to the Ramallah Qalqilya area. Expanding the dataset to include more stations in the West Bank and adding features such as station suggestions based on the user's location is a feature that we plan to implement in the future.
            \item \textbf{Chat:} Implementing a chat feature that allows passengers and drivers to communicate with each other before and during the trip in order to make it easier to coordinate between them is a feature that we plan to implement in the future, as time didn't allow for it in this project.
            \item \textbf{Payment:} Implementing a payment feature that allows passengers to pay for their trips through the app with different payment methods such as reflect, jawalPay, visa and card is a feature that we plan to implement in the future when the app is ready for production and has a large user base.
            \item \textbf{Roads Status Telegram Channel Integration:} Integrating a Telegram channel (احوال الطرق) that provides in time updates on road conditions, closures, and other important information that can help drivers plan their trips more effectively is a feature that we plan to implement in the future.
            \item \textbf{Setting and saving trip routes for drivers:} Implementing a feature that allows drivers to save their trip routes and settings for future trips, so they can be able to create same trips in the future without having to enter the same information again is a feature that we plan to implement in the future.
            \item \textbf{Sharing trip link:} Implementing a feature that allows users to share their trip link using different social media platforms, so they can invite their friends to join their trip is a feature that we plan to implement in the future.
            \item \textbf{Adding more login options:} Implementing more login options such as Google, Facebook, and Apple login is a features and adding a biometric authenticate method such as fingerprint is a feature that we planed to implement but didn't have time for, so we plan to implement it in the future.
            \item \textbf{Adding more languages:} Implementing more languages such as Arabic, and French is a feature that we plan to implement in the future.
            \item \textbf{Adding social and Groups features:} An idea that accord to us was to add simple social features, such as creating groups and weekly scheduled trips, so users can join groups and trips that are scheduled to happen weekly or daily, where the idea accord to us when we thought about company workers, and we thought it would be a great addition to the program, as the thought process was to make the app more social and allow companies to join in, as adding this feature would allow us to add a new type of accounts such as "company account", and make it possible for this user to create a group of its employees who live nearby each others, then they can schedule a trip that happens daily or weekly, and the app will automatically notify the users in the group about the trip, the driver can be someone an employee who has a car, and he is getting payments from the company or the other employees, much more scenarios can be thought of, and we think this feature would be a great addition to the app.
            \item \textbf{Adding a carplay/ android auto feature:} Implementing a feature that allows the app to be used on carplay and android auto, so the driver can use the app on his car screen, and get notifications and updates about the trip, and the passengers can see the trip details on the car screen as this feature would allow him to not use the phone while driving and make it easier for him to see trip status and updates.
        \end{enumerate}
            % \begin{enumerate}
    ...
    \appendix
    \section{Appendix}
        \subsection{Used libraries and packages}
            \begin{enumerate}
                \item \textbf{Flutter:} A UI toolkit for building natively compiled applications for mobile, web, and desktop from a single codebase version 3.22.0.
                \item \textbf{Firebase and its dependecies:} A platform developed by Google for creating mobile and web applications.
                \item \textbf{Google Maps API with google_maps_flutter:} A set of APIs developed by Google that allow you to embed Google Maps into web pages or retrieve data from Google Maps.
                \item \textbf{cupertino_icons:} Cupertino icons as Flutter Icons.
                \item \textbf{riverpod, riverpod_annotation, flutter_riverpod:} A simple way to manage state in Flutter.
                \item \textbf{freezed,freezed_annotation}: A code generator for immutable classes that allows you to write normal data classes without worrying about the serialization.
                \item \textbf{flutter_platform_widgets:} A Flutter plugin that provides specific widgets for each platform.
                \item \textbf{email_validator:} An advanced email validation library.
                \item \textbf{geolocator} A Flutter geolocation plugin that provides easy access to 'platform-specific' location services.
                \item \textbf{tuple} A library that provides a Tuple class.
                \item \textbf{shared_preferences} A Flutter plugin for reading and writing simple key-value pairs into phone storage that works on both iOS and Android.
                \item \textbf{flutter_local_notifications} A Flutter plugin for displaying local notifications and styling them.
